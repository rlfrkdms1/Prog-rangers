...
export const CodeWindow2 = () => {
    const [codeData, setCodeData] = useState({ solution: [] });

    useEffect(() => {

        // const apiUrl = `http://13.124.131.171:8080/api/v1/solutions/${solution.id}`;
      ...
      if (codeData.solution && codeData.solution.length > 0) {
        for (let i = 0; i < codeData.solution.length; i++) {
          const item = codeData.solution[i];
