...
class JwtTokenProviderTest {
    ...
    @Test
    @DisplayName("issuer를 넣지 않으면 ")
    public void 토큰_생성자가_없으면_예외를_반환한다(){
        String accessTokenWithoutIssuer = fakeJwtTokenProvider.createAccessTokenWithoutIssuer(1L);
        assertThatThrownBy(() -> jwtTokenProvider.validToken(accessTokenWithoutIssuer)).isExactlyInstanceOf(MissingIssuerTokenException.class);
